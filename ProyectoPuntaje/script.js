...
document.querySelectorAll('.item').forEach(item => {
    item.addEventListener("mouseover", e =>{
        ...
        document.querySelectorAll(".item").forEach(cuadraditoClarito =>{
            if(cuadraditoClarito.classList.contains('selec')) {
                cuadraditoClarito.classList.remove('selec');
            }
        })
        ...
        puntajeActual = parseFloat(posicion) + 1;
    });
